...
[ -f "$COMPOSE_DIR/.env" ] || { echo "❌ missing $COMPOSE_DIR/.env" >&2; exit 1; }
...
compose up -d --build postgres mongo redis directus userbrew minio imgvault create-buckets 2>&1
...
echo "✅ MinIO ready on :9000"

echo "⏳ Waiting for imgvault..."
until curl -sf http://localhost:8081/health > /dev/null 2>&1; do
  sleep 1
done
echo "✅ imgvault ready on :8081"
